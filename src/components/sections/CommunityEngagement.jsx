        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-8">
            {engagementOutcomes.map((item, index) => (
                <AnimatedCard key={index} delay={index * 150} className="group">
                    <div className={`relative overflow-hidden rounded-3xl shadow-xl hover:shadow-2xl transition-all duration-500 bg-gradient-to-br ${item.bgGradient} px-6 py-2 pt-3 text-white min-h-[300px] flex flex-col justify-between transform group-hover:-translate-y-1`}>
                        {/* Background Pattern */}
                        ...
                        <div className="relative z-10 text-center">
                            <div className="bg-white/20 backdrop-blur-sm rounded-3xl p-6 mb-6 inline-block group-hover:scale-103 transition-transform duration-300">
                                <img
                                    src={item.icon}
                                    alt={`${item.title} icon`}
                            ...
                            <h3 className="text-xl font-bold mb-3 leading-tight text-white ">{item.title}</h3>
